...
level = [
 ...
 # board array imported from board.py
...
class Ghost:
    def __init__(self, x_coord, y_coord, target, speed, img, direct, dead, box, id):
        # Ініціалізація параметрів привида
        self.x_pos = x_coord
        self.y_pos = y_coord
    ...
    def draw(self):
        ...
        # Створення прямокутника для обробки колізій
        ghost_rect = pygame.rect.Rect((self.center_x - 18, self.center_y - 18), (36, 36))
        return ghost_rect

    def check_collisions(self):
        # R, L, U, D
        # Перевірка колізій привида з оточуючим середовищем
        num1 = ((HEIGHT - 50) // 32)
        num2 = (WIDTH // 30)
        num3 = 15
...
run = True
while run:
    timer.tick(fps)  # Управління кількістю кадрів на секунду
    if counter < 19:
        counter += 1
        if counter > 3:
            flicker = False  # Змінна flicker встановлюється в False після 3 кадрів
    else:
        counter = 0
        flicker = True  # Змінна flicker встановлюється в True через кожні 19 кадрів
    if powerup and power_counter < 600:
        power_counter += 1
    elif powerup and power_counter >= 600:
        power_counter = 0
        powerup = False
        eaten_ghost = [False, False, False, False]  # Скидаємо стан вживленості при дотриманні умов
    if startup_counter < 180 and not game_over and not game_won:
        moving = False
        startup_counter += 1
    else:
        moving = True  # Встановлення стану руху гравця

    screen.fill('black')  # Заповнення екрану чорним кольором
    draw_board()  # Малюємо гравця та інші об'єкти
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False  # Вихід з гри при натисканні хрестика вікна        
        if event.type == pygame.KEYDOWN: # Обробка натискань клавіш
    ...
    [7, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 8]
        ...
        if event.type == pygame.KEYUP: # Обробка відпускань клавіш
    ...
    if player_x > 900:
        ...
        player_x = 897 # Зміщення гравця, якщо він вийшов за межі екрану
...
pygame.quit()  # Завершення гри при виході з основного циклу